...
```csharp 1:2 title="Background"
var template = "Hello {name}, what a good day to {action}!";
var document = Document.Create(template).DocumentOrThrow;

var context = Context.CreateBuiltin(new Dictionary<Value, Value>
{
  ["name"] = "Sami",
  ["action"] = "refactor textfit"
});
...
var context = Context.CreateBuiltin(new Dictionary<Value, Value>
{
  [(Value)"name"] = (Value)"Sami",
  [(Value)"action"] = (Value)"refactor textfit"
});
